...
class SupabaseManager:
    ...
    def sign_in(self, email: str, password: str) -> Dict[str, Any]:
        """Sign in existing user (no signup functionality)"""
        try:
...
def render_wordpress_auth(wp_auth: WordPressAuthManager):
    ...
    with tab2:
        ...
        **Alternative Access Methods:**
        - Visit: https://aipropiq.com/wp-login.php
...
def main():
    ...
    if not all([api_key, supabase_url, supabase_key]):
        st.markdown('<h1 class="main-header">🏠 Enhanced RentCast Real Estate Analytics Platform</h1>', unsafe_allow_html=True)
        
        st.markdown("""
        <div class="analysis-section">
            <h2>🚀 Welcome to the Ultimate Real Estate Analytics Platform</h2>
            <p>This comprehensive platform combines advanced property analysis, investment calculations, 
            market comparisons, and WordPress subscription authentication to provide you with the most 
            detailed real estate insights available.</p>
            
            <h3>✨ Key Features:</h3>
            <ul>
                <li>🔍 <strong>Advanced Property Search</strong> - Comprehensive property data lookup</li>
                <li>📊 <strong>Investment Analysis</strong> - Cash flow, ROI, and cap rate calculations</li>
                <li>🏘️ <strong>Market Comparison</strong> - Neighborhood analysis and comparable properties</li>
                <li>📈 <strong>Trend Analysis</strong> - Historical data and appreciation forecasts</li>
                <li>⚠️ <strong>Risk Assessment</strong> - Comprehensive investment risk analysis</li>
                <li>📌 <strong>Property Watchlist</strong> - Save and track favorite properties</li>
                <li>📋 <strong>Detailed Reports</strong> - Export comprehensive analysis reports</li>
                <li>🔐 <strong>WordPress Integration</strong> - Secure subscription-based authentication</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        **🛠️ Setup Instructions:**
        ...
        1. **RentCast API Key**: Get your API key from https://www.rentcast.io/
        2. **Supabase Setup**: Create a project at https://supabase.com/
        3. **Database Tables**: Create the following tables in Supabase:
           - `user_usage` (user_id: text, month: text, usage_count: int, last_usage: timestamp, usage_type: text)
           - `property_searches` (user_id: text, property_data: json, search_date: timestamp, search_type: text, address: text, property_type: text, price: int, bedrooms: int, bathrooms: int, square_footage: int)
           - `property_watchlist` (user_id: text, property_id: text, property_data: json, added_date: timestamp, address: text, price: int, status: text)
           - `analysis_reports` (user_id: text, report_data: json, created_date: timestamp, report_type: text, property_address: text)
           - `wp_users` (user_id: text, username: text, role: text, subscription_id: text, status: text, product_name: text, next_payment_date: text, last_sync: timestamp)
        4. **WordPress Integration**: Ensure your WordPress site has the subscription API endpoint configured
        5. **Enter Credentials**: Fill in all configuration fields in the sidebar
        """)
        return
    ...
    showing_dashboard = render_enhanced_dashboard(user_data, supabase_manager)
    ...
    render_property_search_interface(rentcast_api, supabase_manager, user_id)
