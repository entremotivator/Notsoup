...
class SupabaseManager:
    ...
    def __init__(self, url: str, key: str):
        try:
            ...
            # Test connection
            self.client.table("wp_users").select("count", count="exact").limit(1).execute()
        except Exception as e:
...
def main():
    ...
    if "consumer_secret" not in st.session_state:
        st.session_state.consumer_secret = ""
    if "max_usage" not in st.session_state:
    ...
    wp_consumer_secret = st.sidebar.text_input("🔐 Consumer Secret", type="password", 
                                              help="Required for subscription API access")
    ...
    if wp_username and wp_password and not st.session_state.authenticated:
        if st.sidebar.button("🚀 Quick Login", type="primary"):
            ...
            with st.spinner("Authenticating..."):
                valid, result = wp_auth.check_subscription(wp_username, wp_password, wp_consumer_secret)
                if valid:
                    st.session_state.authenticated = True
                    st.session_state.user_data = result
                    st.session_state.username = wp_username
                    st.session_state.password = wp_password
                    st.session_state.consumer_secret = wp_consumer_secret
    ...
    if not all([api_key, supabase_url, supabase_key]):
        ...
        <div class="analysis-section">
            ...
            powerful real estate investment tools available.</p>
            
            <h3>🔧 Setup Required</h3>
            <p>Please configure the following in the sidebar to get started:</p>
            <ul>
                <li><strong>RentCast API Key:</strong> Your API key for property data access</li>
                <li><strong>Supabase URL & Key:</strong> Database connection for user management</li>
                <li><strong>WordPress Credentials:</strong> Username, password, and consumer secret for authentication</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    with col1:
        st.success(f"✅ Authenticated as: {user_data.get('user_name', 'Unknown')} ({user_data.get('role', 'subscriber')})")
    with col2:
        if st.button("🔄 Refresh Auth"):
            valid, result = wp_auth.check_subscription(
                st.session_state.username, 
                st.session_state.password, 
                st.session_state.consumer_secret
            )
    ...
    with col3:
        if st.button("🚪 Logout"):
            for key in ["authenticated", "user_data", "username", "password", "consumer_secret"]:
